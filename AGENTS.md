...
Conjunction screening (`sat screen`, `compute/conjunction-screening.compute.ts`, envelope `celestrak+spacetrack+gfz+analysis`) correlates SOCRATES conjunctions with the events detector: verdict logic is pure in `domain/conjunction-assessment.ts` — who of the pair can maneuver from SATCAT type/status (debris can't dodge), and a detected maneuver whose window overlaps the days before TCA becomes a `likely-avoidance` verdict inheriting that event's confidence. SOCRATES is the core fetch; the subject's SATCAT record, partner records and element history all degrade with warnings, so the command answers without any account and upgrades with Space-Track credentials (verdicts fall back to `history-unavailable`). Same rule as `sat events`: verdicts always carry evidence and confidence, never bare.
